...
"""Train the ATP_HCR_bin_MutiQ model."""
plt.rcParams['font.sans-serif'] = ['SimHei']
os.environ['HTTP_PROXY'] = 'http://127.0.0.1:7890'
os.environ['HTTPS_PROXY'] = 'http://127.0.0.1:7890'
# Set random seed
def set_seed(seed):
    torch.manual_seed(seed)  # CPU: make random numbers deterministic
    torch.cuda.manual_seed(seed)  # GPU: make random numbers deterministic for the current GPU
...
def fine_mask(image):
    ...
    # Convert to a square region and pad it
    side = max(w, h) + 40
    cx = x + w // 2
    cy = y + h // 2
...
def convert_to_target_path(image_path):
    ...
    # Define base path on the server
    ...
    # Replace "image_2025" with "processed_images" to form the target path
...
def find_and_mask_smallest_region(
        ...
        max_lighten_steps=4,
        lighten_factor=33,
        if_plot=True  # New flag to control visualization
):
    ...
    while lighten_step <= max_lighten_steps:
        # 1. Try the current dominant color
        ...
        # 2. Save the current mask into history
        ...
        lighten_step += 1
        # print(f"Lightening step {lighten_step}: new color {current_color}")

    # All attempts failed
    print("No valid contours found in any attempt")
    if if_plot:
        plt.subplot(1, 3, 2)
        if historical_masks:
...
def compute_bin_edges(mapping_dict, num_bins=8):
    """
    Compute bin edges on log10(ATP) based on the original ATP mapping.
    ...
    # Find the largest bin size and take 10% of it
    max_bin_count = max(bin_counts)
    threshold = max_bin_count * 0.1

    # Accumulate from the first bins until the count exceeds the threshold
    cumulative = 0
    concat_bin_num = 0
...
class MedicalDataset(Dataset):
    def __init__(self, mapping, transform=None, balance_classes=True, bin_edges=None,num_bins=8,concat_bin_num=3):
        ...
        # 3) Record the log10 range for each class
        self.label_ranges = {}
        ...
        # 4) Compute each sample's percentile position within its bin
        self.percentiles = []
    ...
    def save_process_image(self,image,image_raw_path):
        ...
        :return:
        """
        # Save path of processed image, replacing "image_2025" with "processed_images"
        processed_image_path = image_raw_path.replace("image_2025", "processed_images")
        # Create directory
        os.makedirs(os.path.dirname(processed_image_path), exist_ok=True)
        # Ensure image is a NumPy array (OpenCV requirement)
        ...
        # print(f"Save the processed image: {processed_image_path}")

##########################################
# Training/validation loop (aligned with train.py)
##########################################
def train_model(model,
                ...
                model_name="model"):
    """
    Train a model that performs 6-class classification and percentile regression, computing:
      - pct MAE (percentile space)
      - atp MAE (raw ATP space)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
                ...
                for inputs, cls_labels, pct_labels, atp_raw in pbar:
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        for i in range(len(atp_pred)):
                            ...
                            # Skip MAE if the class prediction is wrong
            ...
            # Save the best model (lowest validation raw ATP MAE)
...
class CombinedLoss(nn.Module):
    """
    Combined loss: cross-entropy + α * (SmoothL1 or MSE) regression loss
    """
    ...
    def forward(self, logits, cls_labels, pct_preds, pct_labels):
        ...
        # If pct_preds has shape (B, 1), squeeze it first
...
if __name__ == '__main__':
    ...
    epoch_num = 50
    num_bins = 12
    backbone = 'googlenet'  # Model name
    # 6→4-class, 7→5-class, 8→6-class, 9→7-class, 10→8-class, 11→8-class, 12→9-class, 13→10-class, 14→11-class
    print("Training model name:", backbone)
    ...
    # Save validation mapping to JSON
    ...
    # Assume image size is 224x224 with 3 channels
    ...
    model = train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs=epoch_num, device=str(device),
                        model_name=backbone, label_ranges=train_dataset.label_ranges)
